...
describe Mailboxer::Conversation do
  ...
  it "should be removed from the database once deleted by all participants" do
    ...
    Mailboxer::Conversation.where(id: @conversation.id).exists?.should be_false
  end
  ...
  describe "scopes" do
    ...
    describe ".participant" do
      it "finds conversations with receipts for participant" do
        Mailboxer::Conversation.participant(participant).entries.should == [sentbox_conversation, inbox_conversation]
      end
    end

    describe ".inbox" do
      it "finds inbox conversations with receipts for participant" do
        Mailboxer::Conversation.inbox(participant).entries.should == [inbox_conversation]
      end
    end

    describe ".sentbox" do
      it "finds sentbox conversations with receipts for participant" do
        Mailboxer::Conversation.sentbox(participant).entries.should == [sentbox_conversation]
      end
    end
